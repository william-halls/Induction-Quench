...
## 2026-09-18 - Vault-wide sweep: purge stale NI-9263/NI-9219 hardware references, fix duplicate stale quench-medium note
**Commit**: (pending)
- Full-vault grep found the same "decision changed in one note, never propagated" pattern was far more widespread than the previous two sweeps caught: **NI-9263** (an early hardware plan, replaced by NI-9269 for HOTSHOT power + USB-6009 for all digital/SSR triggering — confirmed via NI-DAQ Control Architecture.md's own "Hardware update" notes) was still stated as current fact, with no caveat, in 10 files: [[Design/Mechanisms/Ball Screw.md]], [[Design/Sample Quenching/INDEX.md]], [[Design/Coil Geometry/INDEX.md]], [[Design/Mechanisms/INDEX.md]], [[Design/Sample Quenching/Quenching Methods.md]], [[Design/Coil Geometry/Round Coil.md]], [[Design/Plumbing/Fluid Systems.md]], [[Design/Wiring/INDEX.md]], [[Design/Wiring/TODO - HOTSHOT Docs & E-Stop-Wiring.md]], and one stray line in [[Design/Wiring/Ball Screw Motor Control.md]] (the rest of that file already correctly documents the hardware history). All corrected to name the actual current module (NI-9269 for HOTSHOT power, NI-9229+CJC amp for thermocouple, USB-6009 for digital triggering) with a note that NI-9263 was the superseded original plan.
- **Verified before fixing, not assumed**: NI-9219 mentions for *pressure monitoring* (in [[Design/Vacuum Chamber/Vacuum Enclosure.md]], [[Design/Plumbing/Air System Control Assembly.md]], and half of the Mechanisms/Quenching Methods INDEX lines) were checked against the NI-DAQ doc and are NOT stale — NI-9219 is explicitly a spare module still earmarked for future pressure-transducer use, so those were left alone. Only NI-9219 mentions describing it as the *current thermocouple input* (Coil Geometry INDEX, Round Coil.md) were actually wrong, since that role moved to NI-9229 + external CJC amp.
- New find: [[Design/Plumbing/Vacuum Lid Systems.md]] is a separate file with near-duplicate content to Fluid Systems.md, including the same "Quench Medium: TBD (oil/water/gas)" staleness already fixed elsewhere in the prior sweep — missed because it wasn't read directly until this pass. Fixed to match the decided water/ball-screw-immersion method.
- Root cause is the same pattern flagged in the prior sweep entry, now confirmed at larger scale: a hardware/decision change in one authoritative note doesn't auto-propagate to the many overview/index notes that reference it. A vault-wide grep for the old identifier (not just re-reading files already "known"), done at the end of a decision-change session, would catch this earlier next time.
...
- [[Design/Mechanisms/Ceramic Mount.md]]: flagged the top-of-file two-part/NPT-thread description as the superseded v1/v2 concept (with a pointer to the current Version 3 section), and struck through the now-superseded "1/4\" NPT" row in the Manufacturing Specifications table.
- [[Design/Plumbing/Fluid Systems.md]]: this note still framed the quench medium/delivery question as fully open ("Decision Pending: whether water stays in the chamber..."), when Quenching Methods.md already has water-via-ball-screw-immersion decided. Added a "Quench delivery (decided)" line and re-scoped the remaining open question to bath-fill timing specifically.
- [[Design/Plumbing/INDEX.md]]: same stale-decision pattern — "Quench Medium: TBD (oil/water/gas)" and spray-nozzle delivery listed as active options. Updated to match the decided water/ball-screw-immersion method; also corrected a stale "NI-9263" pump-control reference to the current USB-6009 digital SSR architecture.
- [[Design/SUBSYSTEMS.md]]: same pattern in the architecture diagram ("Oil/Water/Gas (TBD)") and the "Design Decisions by Subsystem" section ("⏳ Pending: Quench medium selection") — both updated to reflect the decided method. Diagram's sample-mount entry also updated from "(2-part)" to reflect the current Version 3 4-zone design.
- [[README.md]] and [[Induction Quench Research Instrument.md]]: same pattern in the "Key Specifications" tables ("Quench Method: TBD (oil/water/gas options)") — updated to state the decided method with a link to Quenching Methods.md.
- [[README.md]] file-structure tree: was missing four files that exist on disk (Seal Thermal Margin Analysis.md, Material Properties Reference.md, Heat Curve Profile Software.md, Ambrell Contact - HOTSHOT 103927.md) and had Coil Lead Pass-Throughs.md duplicated/misplaced under Plumbing (it only lives under Wiring). Fixed tree, updated file count (50 → 54) and last-updated date; added a new Vault Maintenance log entry (executed via GRUNT — mechanical, no judgment required).
- Deleted `Untitled.md` — empty (0-byte), untracked stray file at vault root, no content, no purpose.
- Root cause pattern across this whole sweep: several facts got corrected in the specific note where the underlying analysis happened, but didn't propagate to the shorter overview/index/hub notes that reference the same fact. Worth grepping for the old value across the vault whenever a hard number or "decided" status changes, rather than relying on catching it later.
